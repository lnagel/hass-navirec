...
from custom_components.navirec.const import API_VERSION  # noqa: E402
...
REQUIRED_SCHEMAS = {
    ...
    "FuelUsedSourceEnum",
    "FuelTypeEnum",
    "ActivityEnum",  # Used by VehicleState.activity
    # Interpretation type for Sensor
    "InterpretationEd4Enum",
}
...
def main() -> int:
    ...
    if not OPENAPI_SPEC.exists():
        ...
        print(f"Download the {API_VERSION} spec to docs/{SPEC_NAME}")
        return 1
    ...
    # Guard against the spec drifting from API_VERSION in const.py
    spec_version = full_spec.get("info", {}).get("version")
    if spec_version != API_VERSION:
        print(
            f"Error: spec declares version {spec_version}, "
            f"but const.py declares API_VERSION {API_VERSION}"
        )
        return 1
    ...
    try:
        ...
        header = f'''"""Auto-generated Pydantic models for Navirec API.

DO NOT EDIT THIS FILE MANUALLY.
Generated from docs/{SPEC_NAME} using:
    uv run python scripts/generate_models.py

To regenerate after API spec updates:
    1. Bump API_VERSION in custom_components/navirec/const.py
    2. Download the matching OpenAPI spec to docs/
